...
def index(request):
    ...
    return render(request, 'index.html', {'user_profile': user_profile, 'posts': feed_lists, 'suggestion_username_profile_list': suggestion_username_profile_list[:4]})
...
def like_post(request):
    ...
    # if it not exists , not cause error
...
def follow(request):
    if request.method == "POST":
        ...
        if FollowerCount.objects.filter(user=user, follower=follower).first():
            delete_follower = FollowerCount.objects.get(user=user, follower=follower)
            delete_follower.delete()
            return redirect('/profile' + user)
        else:
            new_follower = FollowerCount.objects.create(user=user, follower=follower)
            new_follower.save()
            return redirect('profile/' + user)
    else:
        return redirect('/')
